...
function AutorForm({ autorEditando, cargarAutores }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (autorEditando) {
      await axios.put( import.meta.env.VITE_BACKEND_URL + `/api/autores/${autorEditando.id}`, {
        nombre,
        biografia,
      ...
      await axios.post( import.meta.env.VITE_BACKEND_URL + '/api/autores', data);
    }
  ...
  return (
    <form className="form-autor" onSubmit={handleSubmit}>
      ...
      {autorEditando && (
        <img src={ import.meta.env.VITE_BACKEND_URL + `${autorEditando.imagenUrl}`} alt="Vista previa" />
      )}
